...
# Add duplicate columns with different character types to be used when building predictive models
...
levels(dcc$Default1Oct05) <- c("NDef", "Def")
...
# Examine default rates across different variables
# Credit Limit
summary = dcc %>% group_by(LimitAmtBin, Default1Oct05) %>% tally %>% group_by(LimitAmtBin) %>% 
  mutate(pct = n/sum(n), n.pos = cumsum(n) - 0.5*n)
ggplot(summary, aes(x=LimitAmtBin, y=n, fill=Default1Oct05)) + geom_bar(stat="identity", position = position_dodge()) +
...
# Create Logistic model 1, we start with almost all the independent variables
...
# Predicion of Logistic model 4 on Test data
...
# Apply classification tree model on test set and look at accuracy of model
...
#Build random forest model, predict on test data, and check for accuracy
...
RAccuracy = (5540 + 595)/(5540 + 595 + 1064 + 301)
RAccuracy